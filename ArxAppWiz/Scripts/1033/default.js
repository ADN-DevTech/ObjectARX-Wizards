...
function OnFinish (selProj, selObj) {
    try {
		...
		if ( bDotNetModule ) {
			if ( bArxAppType )
				wizard.AddSymbol ("PRJ_TYPE_APP", 'arxnet') ;
			else if ( bDbxAppType )
				wizard.AddSymbol ("PRJ_TYPE_APP", 'dbxnet') ;
			else
				wizard.AddSymbol ("PRJ_TYPE_APP", 'crxnet') ;
			wizard.AddSymbol ("ARX_CLR_SUPPORT", 'true') ;
		} else { 
			if ( bArxAppType )
				wizard.AddSymbol ("PRJ_TYPE_APP", 'arx') ;
			else if ( bDbxAppType )
				wizard.AddSymbol ("PRJ_TYPE_APP", 'dbx') ;
			else
				wizard.AddSymbol ("PRJ_TYPE_APP", 'crx') ;
			wizard.AddSymbol ("ARX_CLR_SUPPORT", 'false') ;
		}

		CopyPropsFile ( 'Autodesk.arx-2017.props', strProjectName) ;
		CopyPropsFile ( 'Autodesk.arx-2017-net.props', strProjectName) ;
