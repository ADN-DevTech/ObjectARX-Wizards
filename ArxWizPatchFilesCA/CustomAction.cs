...
namespace ArxWizPatchFiles {
	...
	public class CustomActions {
        ...
        public static ActionResult PatchPropsWizFiles(Session session)
        {
            ...
            DirectoryInfo di = new DirectoryInfo(TARGETDIR);
            FileInfo[] files = di.GetFiles("*2018*.props", SearchOption.AllDirectories).ToArray();
            ...
            foreach (FileInfo file in files)
            {
                try
                {
                    ...
                    szData = szData.Replace("<AcadDir Condition=\"'$(Platform)'=='x64'\">C:\\Program Files\\Autodesk\\AutoCAD 2018\\</AcadDir>", "<AcadDir Condition=\"'$(Platform)'=='x64'\">" + ACAD + "</AcadDir>");
                    szData = szData.Replace("<AcadDir Condition=\"'$(Platform)'=='Win32'\">C:\\Program Files (x86)\\Autodesk\\AutoCAD 2018\\</AcadDir>", "<AcadDir Condition=\"'$(Platform)'=='Win32'\">" + ACAD + "</AcadDir>");
                    System.IO.File.WriteAllText(file.FullName, szData);
                }
                catch
                {
