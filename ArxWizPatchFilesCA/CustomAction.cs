...
namespace ArxWizPatchFiles {
    ...
    //#endif
        ...
        public static ActionResult PatchPropsWizFiles(Session session)
        {
            ...
            //C:\Program Files (x86)\Autodesk\ObjectARX 2023 Wizards\
            string ACAD = session["ACAD"];
            session.Log(" >> PatchPropsWizFiles: ACAD = " + ACAD);

            DirectoryInfo di = new DirectoryInfo(TARGETDIR);
            FileInfo[] files = di.GetFiles("*2023*.props", SearchOption.AllDirectories).ToArray();
            ...
            foreach (FileInfo file in files)
            {
                try
                {
                    ...
                    szData = szData.Replace("<AcadDir Condition=\"'$(Platform)'=='x64'\">C:\\Program Files\\Autodesk\\AutoCAD 2023\\</AcadDir>", "<AcadDir Condition=\"'$(Platform)'=='x64'\">" + ACAD + "</AcadDir>");
                    szData = szData.Replace("<AcadDir Condition=\"'$(Platform)'=='Win32'\">C:\\Program Files (x86)\\Autodesk\\AutoCAD 2023\\</AcadDir>", "<AcadDir Condition=\"'$(Platform)'=='Win32'\">" + ACAD + "</AcadDir>");
                    System.IO.File.WriteAllText(file.FullName, szData);
                }
                catch
                {
