...
[!output CLASS_NAME_ROOT]::[!output CLASS_NAME_ROOT] (const bool autoInitAndRelease) : [!output BASE_CLASS](), mbAutoInitAndRelease(autoInitAndRelease) {
	if ( autoInitAndRelease ) {
		if ( acdbSummaryInfoManager () )
			acdbSummaryInfoManager ()->addReactor (this) ;
		else
...
void [!output CLASS_NAME_ROOT]::Detach () {
	if ( mbAutoInitAndRelease ) {
		if ( acdbSummaryInfoManager () ) {
			acdbSummaryInfoManager ()->removeReactor (this) ;
			mbAutoInitAndRelease =false ;
		}
	}
}

AcDbSummaryInfoManager *[!output CLASS_NAME_ROOT]::Subject () const {
	return (acdbSummaryInfoManager ()) ;
}
